...
//    public static final String DELETE = "DELETE";
    ...
    public Map<String, Object> executeByMap(String url, Map<String, Object> params) throws ServiceException {
        ...
        try {
            ...
            if (!Strings.isNullOrEmpty(swagger.getBasePath()))
            ...
            */
            ...
            Map<String, Object> results = postProcessResult(acceptedStatus, operation, response);
    ...
    protected static Map<String,Object> postProcessResult(String acceptedStatus, Operation operation, HttpResponse response) throws ServiceException {
        ...
        if (entity.getContentType()!=null &&  RestUtils.checkContentType(entity.getContentType().getValue(), APPLICATION_OCTET_STREAM))
        {
            try {
        ...
        else if (outputIsFile)
        {
            if (entity.getContentType()!=null && RestUtils.checkContentType(entity.getContentType().getValue(), RestUtils.MULTIPART))
            {
                ...
                try {
                    ...
                    for( int i=0; i<list.getCount(); i++)
                    {
                        ...
                        String tempFile = RestUtils.toFile(item.getInputStream(),item.getFileName());
            ...
            else
            {
                try {
                    ...
                    {
                        result = EntityUtils.toString(entity, RestUtils.DEFAULT_CHARSET);
                        byte[] bytes = Base64.decodeBase64((String) result);
                        result = RestUtils.toFile(new ByteArrayInputStream(bytes),filename);
                    }
                    else
                    {
                        result = RestUtils.toFile(entity.getContent(),filename);
                    }
